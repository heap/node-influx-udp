...
InfluxUdp.prototype.send = function influxSend(points, callback) {
    var message = this.encode(points);
    var socket = dgram.createSocket(this.socketType);

    socket.send(message, 0, message.length, this.port, this.host, function (err) {
